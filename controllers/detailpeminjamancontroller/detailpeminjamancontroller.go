...
func ShowByIdUser(c *gin.Context) {
	var detailPeminjaman models.DetailPeminjaman
	id := c.Param("id")
	if err := models.DB.Where("id_user = ?", id).Order("id desc").Limit(1).First(&detailPeminjaman, id).Error; err != nil {
		switch err {
		case gorm.ErrRecordNotFound:
			c.AbortWithStatusJSON(http.StatusNotFound, gin.H{"error": "Record not found!"})
			return 
...
func HistoryUser(c *gin.Context){
	...
	if err := models.DB.Where("id_user = ? AND status = ? OR  status = ?", id, "done", "Di Batalkan").Find(&detailPeminjaman).Error; err != nil {
		c.AbortWithStatusJSON(http.StatusNotFound, gin.H{"error": "Record not found!"})
	}
